...
const handleS3FileSelect = async (fileKey) => {
    ...
    switch (data.type) {
       case 'chat':
        if (data.chat.is_flashcard) {
          ...
          setShowAnswer(false);
          // Add this line to ensure the flashcard chat appears in the list
          setChats(prevChats => {
  ...
  const sendWebSocketMessage = (type, payload) => {
    if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
      const message = JSON.stringify({ type, token, llm_type: llmType,role :'student', ...payload });
  ...
  const handleSubmit = async (e) => {
    ...
    if (input.trim()) {
      ...
      // Optimized state update: pass a function to setMessages to ensure proper state updates
      setMessages(prevMessages => [...prevMessages, userMessage]);
      
      // Clear input field after user submits the message
      setInput('');
  
      try {
        // Handle LinkedIn Session
        ...
        // Handle Flashcard Mode
          ...
          if (response.ok) {
            ...
            alert("Error querying Flashcard RAG. Please try again.");
          }
  
        // Handle WebSocket logic for general chat if other modes are not active
        } else {
          if (!currentChat) {
          ...
          // Send the WebSocket message with user input
...
<form onSubmit={handleSubmit} className="chat-input-form">
          ...
          <input
            ...
            placeholder={pdfRagMode ? "Ask a question about the PDF..." : "Type your message..."}
